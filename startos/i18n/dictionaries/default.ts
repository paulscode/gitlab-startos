...
const dict = {
  ...
  'Create Runner Token (automatic)': 8,
  ...
  'The address GitLab treated as canonical is no longer available. It is now using a different one — confirm or change it.': 41,
  'This replaces the current root password immediately. Anyone relying on the old one will be locked out.': 42,
  'Upgrade Blocked': 43,
  'Used by the GitLab Runner package to register itself. Creates an untagged runner that accepts any job.': 44,
  'Web Interface': 45,
  'Web UI and Git over HTTPS': 46,
} as const
