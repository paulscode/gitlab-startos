...
| Architectures | x86_64 (tested), aarch64 (published, **untested** — see Limitations) |
...
9. **Updates cannot always skip releases.** GitLab enforces a minimum version to upgrade from, so a long-dormant instance may need an intermediate release installed before the newest one. The package checks this before starting and names the required version rather than failing obscurely.
10. **aarch64 is published but untested.** The build is produced from upstream's official ARM image, but it has never been run on ARM hardware, and GitLab documents outstanding issues on that architecture — historically including a Redis copy-on-write bug, and Redis is bundled here. Treat it as provisional: take a backup before depending on it. The x86_64 build is tested.
11. **An external PostgreSQL or Redis cannot be used.** The bundled ones are not optional through this package.
